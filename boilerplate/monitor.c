...
struct monitored_entry {
    ...
    int            soft_warned;          /* non-zero once warning emitted */
...
/* hrtimer / timer_list callbacks run in process context since         */
/* Linux 4.15 when used with mod_timer().  More importantly:           */
/*   - copy_from_user() in the ioctl path cannot be called with a     */
/*     spinlock held (it may sleep on a page fault).                   */
/*   - get_task_mm() / mmput() in get_rss_bytes can also schedule.    */
/* Therefore a mutex is the correct primitive here.                    */
/* ------------------------------------------------------------------ */
...
static void timer_callback(struct timer_list *t)
{
    ...
    /*
     * We use mutex_trylock so the timer never deadlocks with the ioctl
     * path.  If the lock is contended, we simply retry on the next tick.
     */
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        /* Hard limit check (enforced before soft to avoid double action) */
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        if (req.soft_limit_bytes > req.hard_limit_bytes) {
            printk(KERN_WARNING
                   "[container_monitor] Register failed: soft > hard for pid=%d\n",
                   req.pid);
            return -EINVAL;
        }
